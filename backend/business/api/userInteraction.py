...
def like_paper(request):
    ...
    if request.method == 'POST':
        ...
        if user and paper:
            ...
            return JsonResponse({'error': '用户或文献不存在'}, status=400)
    else:
...
def score_paper(request):
    ...
    if request.method == 'POST':
        ...
        # 判断用户是否对该文献进行过评分
        if paper_score:
            return JsonResponse({'error': '用户已对该文献进行过评分'}, status=400)
        # 判断评分是否在0到10之间，且为整数
        if not isinstance(score, int) or score < 0 or score > 10:
            return JsonResponse({'error': '评分应为0到10之间的整数'}, status=400)
        # 存储评分，更新文献平均分，保留两位小数
        if user and paper:
            ...
            paper.save()
            return JsonResponse({'message': '评分成功'})
        else:
            return JsonResponse({'error': '用户或文献不存在'}, status=400)
    else:
        return JsonResponse({'error': '请求方法错误'}, status=400)
